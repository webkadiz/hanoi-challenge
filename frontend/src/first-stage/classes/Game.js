...
export default class Game {
  ...
  computeValuesDependOnAmountRings() {
    ...
    this.maxWidthRing =
      this.increaseWidthRing * this.amountRings + this.baseWidthRing
  ...
  computeDimensionsGlow() {
    for (const i of range(3)) {
      ...
      this.map.glows
        .eq(i)
        .css("height", this.baseHeightRing * this.amountRings + 100)
  ...
  setRodsLeft() {
    for (const i of range(3)) {
      this.map.rods
        ...
        .css(
          "left",
          this.edgeMargin + i * this.ableWidthForRods - this.rodWidth / 2
        )
    }
  }
    ...
    if (UP) {
      ...
      if (this.activeRing.exists()) {
        ...
        if (topRing) {
          this.activeRing
            .get()
            .animate({ bottom: topRing.bottom + topRing.height })
        } else {
      ...
      if (this.activeRing.exists()) {
        ...
        if (rightRingOffset + this.ableWidthForRods < rightRodOffset) {
          this.activeRing
            .get()
            .animate({ left: rightRingOffset + this.ableWidthForRods })
        }
      } else {
      }
      ...
      if (this.activeRing.exists()) {
        ...
        if (
          leftRingOffset -
            this.ableWidthForRods +
            this.activeRing.get().width / 2 >
          leftRodOffset
        ) {
          this.activeRing
            .get()
            .animate({ left: leftRingOffset - this.ableWidthForRods })
        }
      }
